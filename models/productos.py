...
class Productos:
    ...
    def consultar_productos(self):
            sql = f"SELECT productos.contador, nombre_producto, emprendedor_unidad, precio_producto, presentacion_producto, cantidad_disponible, imagen FROM productos INNER JOIN emprendedores ON productos.codigo_unidad = emprendedores.codigo_unidad WHERE cantidad_disponible > 0 ORDER BY nombre_producto;"
    ...
    def consultar_productos_busqueda_categoria(self, cate):
            sql = f"SELECT productos.contador, nombre_producto, emprendedor_unidad, precio_producto, presentacion_producto, cantidad_disponible, imagen FROM productos INNER JOIN emprendedores ON productos.codigo_unidad = emprendedores.codigo_unidad WHERE categoria = '{cate}' AND cantidad_disponible > 0 ORDER BY nombre_producto"
